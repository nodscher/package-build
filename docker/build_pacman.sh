...
if [[ $(basename $0) == build-mesa ]]; then
    ...
    [[ "$(tar --exclude='*/*' -tf repo/custom/custom.db.tar.zst | grep mesa-git)" != *"$latest"* ]] || (echo mesa-git-$latest is already up to date; exit 1)
    build-mesa
elif [[ $(basename $0) == build-linux-drm ]]; then
    latest=$(git ls-remote https://gitlab.freedesktop.org/drm/tip.git HEAD | cut -c1-9)
    [[ "$(tar --exclude='*/*' -tf repo/custom/custom.db.tar.zst | grep linux-drm-tip-git-)" != *"$latest"* ]] || (echo linux-drm-tip-git-$latest is already up to date; exit 1)
    build-linux-drm
elif [[ $(basename $0) == build-linux-mainline ]]; then
    latest=$(cat kernel/linux-mainline/PKGBUILD | grep pkgver= | cut -d= -f2)
    [[ "$(tar --exclude='*/*' -tf repo/custom/custom.db.tar.zst | grep linux-mainline)" != *"$latest"* ]] || (echo linux-mainline-$latest is already up to date; exit 1)
    build-linux-mainline
else
